...
// SetupGated adds a controller that reconciles Infrastructure managed resources.
func SetupGated(mgr ctrl.Manager, o tjcontroller.Options) error {
	o.Options.Gate.Register(func() {
		if err := Setup(mgr, o); err != nil {
			mgr.GetLogger().Error(err, "unable to setup reconciler", "gvk", v1alpha1.Infrastructure_GroupVersionKind.String())
		}
	}, v1alpha1.Infrastructure_GroupVersionKind)
	return nil
}

// Setup adds a controller that reconciles Infrastructure managed resources.
func Setup(mgr ctrl.Manager, o tjcontroller.Options) error {
	name := managed.ControllerName(v1alpha1.Infrastructure_GroupVersionKind.String())
	var initializers managed.InitializerChain
	eventHandler := handler.NewEventHandler(handler.WithLogger(o.Logger.WithValues("gvk", v1alpha1.Infrastructure_GroupVersionKind)))
	ac := tjcontroller.NewAPICallbacks(mgr, xpresource.ManagedKind(v1alpha1.Infrastructure_GroupVersionKind), tjcontroller.WithEventHandler(eventHandler), tjcontroller.WithStatusUpdates(false))
	metricRecorder := metrics.NewMetricRecorder(v1alpha1.Infrastructure_GroupVersionKind, mgr, o.PollInterval)
	if err := mgr.Add(metricRecorder); err != nil {
		return errors.Wrap(err, "cannot register Terraform metric recorder")
	}
	opts := []managed.ReconcilerOption{
		managed.WithExternalConnecter(
			tjcontroller.NewTerraformPluginSDKAsyncConnector(mgr.GetClient(), o.OperationTrackerStore, o.SetupFn, o.Provider.Resources["oci_datacc_infrastructure"],
	...
	// register webhooks for the kind v1alpha1.Infrastructure
	// if they're enabled.
	if o.StartWebhooks {
		if err := ctrl.NewWebhookManagedBy(mgr).
			For(&v1alpha1.Infrastructure{}).
			Complete(); err != nil {
			return errors.Wrap(err, "cannot register webhook for the kind v1alpha1.Infrastructure")
		}
	}
	...
	r := managed.NewReconciler(mgr, xpresource.ManagedKind(v1alpha1.Infrastructure_GroupVersionKind), opts...)

	return ctrl.NewControllerManagedBy(mgr).
		Named(name).
		WithOptions(o.ForControllerRuntime()).
		WithEventFilter(xpresource.DesiredStateChanged()).
		Watches(&v1alpha1.Infrastructure{}, eventHandler).
