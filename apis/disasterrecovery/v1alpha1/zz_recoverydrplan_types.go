...
type RecoveryDrPlanInitParameters struct {
...
type RecoveryDrPlanObservation struct {
...
type RecoveryDrPlanParameters struct {
...
// RecoveryDrPlanSpec defines the desired state of RecoveryDrPlan
type RecoveryDrPlanSpec struct {
	v1.ResourceSpec `json:",inline"`
	ForProvider     RecoveryDrPlanParameters `json:"forProvider"`
	...
	// autoscaler.
	InitProvider RecoveryDrPlanInitParameters `json:"initProvider,omitempty"`
}

// RecoveryDrPlanStatus defines the observed state of RecoveryDrPlan.
type RecoveryDrPlanStatus struct {
	v1.ResourceStatus `json:",inline"`
	AtProvider        RecoveryDrPlanObservation `json:"atProvider,omitempty"`
}
...
// RecoveryDrPlan is the Schema for the RecoveryDrPlans API. Provides the Dr Plan resource in Oracle Cloud Infrastructure Disaster Recovery service
// +kubebuilder:printcolumn:name="SYNCED",type="string",JSONPath=".status.conditions[?(@.type=='Synced')].status"
// +kubebuilder:printcolumn:name="READY",type="string",JSONPath=".status.conditions[?(@.type=='Ready')].status"
// +kubebuilder:printcolumn:name="EXTERNAL-NAME",type="string",JSONPath=".metadata.annotations.crossplane\\.io/external-name"
// +kubebuilder:printcolumn:name="AGE",type="date",JSONPath=".metadata.creationTimestamp"
// +kubebuilder:resource:scope=Cluster,categories={crossplane,managed,oci}
type RecoveryDrPlan struct {
	metav1.TypeMeta   `json:",inline"`
	metav1.ObjectMeta `json:"metadata,omitempty"`
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.displayName) || (has(self.initProvider) && has(self.initProvider.displayName))",message="spec.forProvider.displayName is a required parameter"
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.drProtectionGroupId) || (has(self.initProvider) && has(self.initProvider.drProtectionGroupId))",message="spec.forProvider.drProtectionGroupId is a required parameter"
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.type) || (has(self.initProvider) && has(self.initProvider.type))",message="spec.forProvider.type is a required parameter"
	Spec   RecoveryDrPlanSpec   `json:"spec"`
	Status RecoveryDrPlanStatus `json:"status,omitempty"`
}

// +kubebuilder:object:root=true

// RecoveryDrPlanList contains a list of RecoveryDrPlans
type RecoveryDrPlanList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []RecoveryDrPlan `json:"items"`
}

// Repository type metadata.
var (
	RecoveryDrPlan_Kind             = "RecoveryDrPlan"
	RecoveryDrPlan_GroupKind        = schema.GroupKind{Group: CRDGroup, Kind: RecoveryDrPlan_Kind}.String()
	RecoveryDrPlan_KindAPIVersion   = RecoveryDrPlan_Kind + "." + CRDGroupVersion.String()
	RecoveryDrPlan_GroupVersionKind = CRDGroupVersion.WithKind(RecoveryDrPlan_Kind)
)

func init() {
	SchemeBuilder.Register(&RecoveryDrPlan{}, &RecoveryDrPlanList{})
}
