/*
Copyright 2022 Upbound Inc.
*/

// Code generated by upjet. DO NOT EDIT.

package v1alpha1

import (
	metav1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	"k8s.io/apimachinery/pkg/runtime/schema"

	v1 "github.com/crossplane/crossplane-runtime/apis/common/v1"
)

type DatastoresInitParameters struct {

	// A list of OCIDs of Block Storage Volumes.
	BlockVolumeIds []*string `json:"blockVolumeIds,omitempty" tf:"block_volume_ids,omitempty"`

	// Type of the datastore.
	DatastoreType *string `json:"datastoreType,omitempty" tf:"datastore_type,omitempty"`
}

type DatastoresObservation struct {

	// A list of OCIDs of Block Storage Volumes.
	BlockVolumeIds []*string `json:"blockVolumeIds,omitempty" tf:"block_volume_ids,omitempty"`

	// Size of the Block Storage Volume in GB.
	Capacity *float64 `json:"capacity,omitempty" tf:"capacity,omitempty"`

	// Type of the datastore.
	DatastoreType *string `json:"datastoreType,omitempty" tf:"datastore_type,omitempty"`
}

type DatastoresParameters struct {

	// A list of OCIDs of Block Storage Volumes.
	// +kubebuilder:validation:Optional
	BlockVolumeIds []*string `json:"blockVolumeIds" tf:"block_volume_ids,omitempty"`

	// Type of the datastore.
	// +kubebuilder:validation:Optional
	DatastoreType *string `json:"datastoreType" tf:"datastore_type,omitempty"`
}

type NetworkConfigurationInitParameters struct {

	// (Updatable) The OCID of the VLAN used by the SDDC for the HCX component of the VMware environment. This VLAN is a mandatory attribute  for Management Cluster when HCX is enabled.
	HcxVlanID *string `json:"hcxVlanId,omitempty" tf:"hcx_vlan_id,omitempty"`

	// (Updatable) The OCID of the VLAN used by the SDDC for the NSX Edge Uplink 1 component of the VMware environment. This VLAN is a mandatory attribute for Management Cluster.
	NsxEdgeUplink1VlanID *string `json:"nsxEdgeUplink1VlanId,omitempty" tf:"nsx_edge_uplink1vlan_id,omitempty"`

	// (Updatable) The OCID of the VLAN used by the SDDC  for the NSX Edge Uplink 2 component of the VMware environment. This VLAN is a mandatory attribute for Management Cluster.
	NsxEdgeUplink2VlanID *string `json:"nsxEdgeUplink2VlanId,omitempty" tf:"nsx_edge_uplink2vlan_id,omitempty"`

	// (Updatable) The OCID of the VLAN used by the Cluster for the NSX Edge VTEP component of the VMware environment.
	NsxEdgeVtepVlanID *string `json:"nsxEdgeVtepVlanId,omitempty" tf:"nsx_edge_vtep_vlan_id,omitempty"`

	// (Updatable) The OCID of the VLAN used by the Cluster for the NSX VTEP component of the VMware environment.
	NsxVtepVlanID *string `json:"nsxVtepVlanId,omitempty" tf:"nsx_vtep_vlan_id,omitempty"`

	// (Updatable) The OCID of the management subnet used to provision the Cluster.
	ProvisioningSubnetID *string `json:"provisioningSubnetId,omitempty" tf:"provisioning_subnet_id,omitempty"`

	// (Updatable) The OCID of the VLAN used by the Cluster for the Provisioning component of the VMware environment.
	ProvisioningVlanID *string `json:"provisioningVlanId,omitempty" tf:"provisioning_vlan_id,omitempty"`

	// (Updatable) The OCID of the VLAN used by the Cluster for the vSphere Replication component of the VMware environment.
	ReplicationVlanID *string `json:"replicationVlanId,omitempty" tf:"replication_vlan_id,omitempty"`

	// (Updatable) The OCID of the VLAN used by the Cluster for the vMotion component of the VMware environment.
	VmotionVlanID *string `json:"vmotionVlanId,omitempty" tf:"vmotion_vlan_id,omitempty"`

	// (Updatable) The OCID of the VLAN used by the Cluster for the vSAN component of the VMware environment.
	VsanVlanID *string `json:"vsanVlanId,omitempty" tf:"vsan_vlan_id,omitempty"`

	// (Updatable) The OCID of the VLAN used by the SDDC for the vSphere component of the VMware environment. This VLAN is a mandatory attribute for Management Cluster.
	VsphereVlanID *string `json:"vsphereVlanId,omitempty" tf:"vsphere_vlan_id,omitempty"`
}

type NetworkConfigurationObservation struct {

	// (Updatable) The OCID of the VLAN used by the SDDC for the HCX component of the VMware environment. This VLAN is a mandatory attribute  for Management Cluster when HCX is enabled.
	HcxVlanID *string `json:"hcxVlanId,omitempty" tf:"hcx_vlan_id,omitempty"`

	// (Updatable) The OCID of the VLAN used by the SDDC for the NSX Edge Uplink 1 component of the VMware environment. This VLAN is a mandatory attribute for Management Cluster.
	NsxEdgeUplink1VlanID *string `json:"nsxEdgeUplink1VlanId,omitempty" tf:"nsx_edge_uplink1vlan_id,omitempty"`

	// (Updatable) The OCID of the VLAN used by the SDDC  for the NSX Edge Uplink 2 component of the VMware environment. This VLAN is a mandatory attribute for Management Cluster.
	NsxEdgeUplink2VlanID *string `json:"nsxEdgeUplink2VlanId,omitempty" tf:"nsx_edge_uplink2vlan_id,omitempty"`

	// (Updatable) The OCID of the VLAN used by the Cluster for the NSX Edge VTEP component of the VMware environment.
	NsxEdgeVtepVlanID *string `json:"nsxEdgeVtepVlanId,omitempty" tf:"nsx_edge_vtep_vlan_id,omitempty"`

	// (Updatable) The OCID of the VLAN used by the Cluster for the NSX VTEP component of the VMware environment.
	NsxVtepVlanID *string `json:"nsxVtepVlanId,omitempty" tf:"nsx_vtep_vlan_id,omitempty"`

	// (Updatable) The OCID of the management subnet used to provision the Cluster.
	ProvisioningSubnetID *string `json:"provisioningSubnetId,omitempty" tf:"provisioning_subnet_id,omitempty"`

	// (Updatable) The OCID of the VLAN used by the Cluster for the Provisioning component of the VMware environment.
	ProvisioningVlanID *string `json:"provisioningVlanId,omitempty" tf:"provisioning_vlan_id,omitempty"`

	// (Updatable) The OCID of the VLAN used by the Cluster for the vSphere Replication component of the VMware environment.
	ReplicationVlanID *string `json:"replicationVlanId,omitempty" tf:"replication_vlan_id,omitempty"`

	// (Updatable) The OCID of the VLAN used by the Cluster for the vMotion component of the VMware environment.
	VmotionVlanID *string `json:"vmotionVlanId,omitempty" tf:"vmotion_vlan_id,omitempty"`

	// (Updatable) The OCID of the VLAN used by the Cluster for the vSAN component of the VMware environment.
	VsanVlanID *string `json:"vsanVlanId,omitempty" tf:"vsan_vlan_id,omitempty"`

	// (Updatable) The OCID of the VLAN used by the SDDC for the vSphere component of the VMware environment. This VLAN is a mandatory attribute for Management Cluster.
	VsphereVlanID *string `json:"vsphereVlanId,omitempty" tf:"vsphere_vlan_id,omitempty"`
}

type NetworkConfigurationParameters struct {

	// (Updatable) The OCID of the VLAN used by the SDDC for the HCX component of the VMware environment. This VLAN is a mandatory attribute  for Management Cluster when HCX is enabled.
	// +kubebuilder:validation:Optional
	HcxVlanID *string `json:"hcxVlanId,omitempty" tf:"hcx_vlan_id,omitempty"`

	// (Updatable) The OCID of the VLAN used by the SDDC for the NSX Edge Uplink 1 component of the VMware environment. This VLAN is a mandatory attribute for Management Cluster.
	// +kubebuilder:validation:Optional
	NsxEdgeUplink1VlanID *string `json:"nsxEdgeUplink1VlanId,omitempty" tf:"nsx_edge_uplink1vlan_id,omitempty"`

	// (Updatable) The OCID of the VLAN used by the SDDC  for the NSX Edge Uplink 2 component of the VMware environment. This VLAN is a mandatory attribute for Management Cluster.
	// +kubebuilder:validation:Optional
	NsxEdgeUplink2VlanID *string `json:"nsxEdgeUplink2VlanId,omitempty" tf:"nsx_edge_uplink2vlan_id,omitempty"`

	// (Updatable) The OCID of the VLAN used by the Cluster for the NSX Edge VTEP component of the VMware environment.
	// +kubebuilder:validation:Optional
	NsxEdgeVtepVlanID *string `json:"nsxEdgeVtepVlanId" tf:"nsx_edge_vtep_vlan_id,omitempty"`

	// (Updatable) The OCID of the VLAN used by the Cluster for the NSX VTEP component of the VMware environment.
	// +kubebuilder:validation:Optional
	NsxVtepVlanID *string `json:"nsxVtepVlanId" tf:"nsx_vtep_vlan_id,omitempty"`

	// (Updatable) The OCID of the management subnet used to provision the Cluster.
	// +kubebuilder:validation:Optional
	ProvisioningSubnetID *string `json:"provisioningSubnetId" tf:"provisioning_subnet_id,omitempty"`

	// (Updatable) The OCID of the VLAN used by the Cluster for the Provisioning component of the VMware environment.
	// +kubebuilder:validation:Optional
	ProvisioningVlanID *string `json:"provisioningVlanId,omitempty" tf:"provisioning_vlan_id,omitempty"`

	// (Updatable) The OCID of the VLAN used by the Cluster for the vSphere Replication component of the VMware environment.
	// +kubebuilder:validation:Optional
	ReplicationVlanID *string `json:"replicationVlanId,omitempty" tf:"replication_vlan_id,omitempty"`

	// (Updatable) The OCID of the VLAN used by the Cluster for the vMotion component of the VMware environment.
	// +kubebuilder:validation:Optional
	VmotionVlanID *string `json:"vmotionVlanId" tf:"vmotion_vlan_id,omitempty"`

	// (Updatable) The OCID of the VLAN used by the Cluster for the vSAN component of the VMware environment.
	// +kubebuilder:validation:Optional
	VsanVlanID *string `json:"vsanVlanId" tf:"vsan_vlan_id,omitempty"`

	// (Updatable) The OCID of the VLAN used by the SDDC for the vSphere component of the VMware environment. This VLAN is a mandatory attribute for Management Cluster.
	// +kubebuilder:validation:Optional
	VsphereVlanID *string `json:"vsphereVlanId,omitempty" tf:"vsphere_vlan_id,omitempty"`
}

type OcvsClusterInitParameters struct {

	// The OCID of the Capacity Reservation.
	CapacityReservationID *string `json:"capacityReservationId,omitempty" tf:"capacity_reservation_id,omitempty"`

	// The availability domain to create the Cluster's ESXi hosts in. For multi-AD Cluster deployment, set to multi-AD.
	ComputeAvailabilityDomain *string `json:"computeAvailabilityDomain,omitempty" tf:"compute_availability_domain,omitempty"`

	// A list of datastore info for the Cluster. This value is required only when initialHostShapeName is a standard shape.
	Datastores []DatastoresInitParameters `json:"datastores,omitempty" tf:"datastores,omitempty"`

	// (Updatable) Defined tags for this resource. Each key is predefined and scoped to a namespace. For more information, see Resource Tags.  Example: {"Operations.CostCenter": "42"}
	// +mapType=granular
	DefinedTags map[string]*string `json:"definedTags,omitempty" tf:"defined_tags,omitempty"`

	// (Updatable) A descriptive name for the Cluster. Cluster name requirements are 1-16 character length limit, Must start with a letter, Must be English letters, numbers, - only, No repeating hyphens, Must be unique within the region. Avoid entering confidential information.
	DisplayName *string `json:"displayName,omitempty" tf:"display_name,omitempty"`

	// The number of ESXi hosts to create in the Cluster. You can add more hosts later (see CreateEsxiHost).
	EsxiHostsCount *float64 `json:"esxiHostsCount,omitempty" tf:"esxi_hosts_count,omitempty"`

	// (Updatable) The ESXi software bundle to install on the ESXi hosts in the Cluster.  Only versions under the same vmwareSoftwareVersion and have been validate by Oracle Cloud VMware Solution will be accepted. To get a list of the available versions, use ListSupportedVmwareSoftwareVersions.
	EsxiSoftwareVersion *string `json:"esxiSoftwareVersion,omitempty" tf:"esxi_software_version,omitempty"`

	// (Updatable) Free-form tags for this resource. Each tag is a simple key-value pair with no predefined name, type, or namespace. For more information, see Resource Tags.  Example: {"Department": "Finance"}
	// +mapType=granular
	FreeformTags map[string]*string `json:"freeformTags,omitempty" tf:"freeform_tags,omitempty"`

	// The billing option selected during Cluster creation. ListSupportedCommitments.
	InitialCommitment *string `json:"initialCommitment,omitempty" tf:"initial_commitment,omitempty"`

	// The initial OCPU count of the Cluster's ESXi hosts.
	InitialHostOcpuCount *float64 `json:"initialHostOcpuCount,omitempty" tf:"initial_host_ocpu_count,omitempty"`

	// The initial compute shape of the Cluster's ESXi hosts. ListSupportedHostShapes.
	InitialHostShapeName *string `json:"initialHostShapeName,omitempty" tf:"initial_host_shape_name,omitempty"`

	// A prefix used in the name of each ESXi host and Compute instance in the Cluster. If this isn't set, the Cluster's displayName is used as the prefix.
	InstanceDisplayNamePrefix *string `json:"instanceDisplayNamePrefix,omitempty" tf:"instance_display_name_prefix,omitempty"`

	// Indicates whether shielded instance is enabled for this Cluster.
	IsShieldedInstanceEnabled *bool `json:"isShieldedInstanceEnabled,omitempty" tf:"is_shielded_instance_enabled,omitempty"`

	// (Updatable) The network configurations used by Cluster, including OCIDs of the management subnet and VLANs.
	NetworkConfiguration []NetworkConfigurationInitParameters `json:"networkConfiguration,omitempty" tf:"network_configuration,omitempty"`

	// The OCID of the SDDC that the Cluster belongs to.
	SddcID *string `json:"sddcId,omitempty" tf:"sddc_id,omitempty"`

	// (Updatable) The VMware software bundle to install on the ESXi hosts in the Cluster. To get a list of the available versions, use ListSupportedVmwareSoftwareVersions.
	VmwareSoftwareVersion *string `json:"vmwareSoftwareVersion,omitempty" tf:"vmware_software_version,omitempty"`

	// The CIDR block for the IP addresses that VMware VMs in the Cluster use to run application workloads.
	WorkloadNetworkCidr *string `json:"workloadNetworkCidr,omitempty" tf:"workload_network_cidr,omitempty"`
}

type OcvsClusterObservation struct {

	// The number of ESXi hosts to create in the Cluster. You can add more hosts later (see CreateEsxiHost).
	ActualEsxiHostsCount *float64 `json:"actualEsxiHostsCount,omitempty" tf:"actual_esxi_hosts_count,omitempty"`

	// The OCID of the Capacity Reservation.
	CapacityReservationID *string `json:"capacityReservationId,omitempty" tf:"capacity_reservation_id,omitempty"`

	// The OCID of the compartment that contains the Cluster.
	CompartmentID *string `json:"compartmentId,omitempty" tf:"compartment_id,omitempty"`

	// The availability domain to create the Cluster's ESXi hosts in. For multi-AD Cluster deployment, set to multi-AD.
	ComputeAvailabilityDomain *string `json:"computeAvailabilityDomain,omitempty" tf:"compute_availability_domain,omitempty"`

	// A list of datastore info for the Cluster. This value is required only when initialHostShapeName is a standard shape.
	Datastores []DatastoresObservation `json:"datastores,omitempty" tf:"datastores,omitempty"`

	// (Updatable) Defined tags for this resource. Each key is predefined and scoped to a namespace. For more information, see Resource Tags.  Example: {"Operations.CostCenter": "42"}
	// +mapType=granular
	DefinedTags map[string]*string `json:"definedTags,omitempty" tf:"defined_tags,omitempty"`

	// (Updatable) A descriptive name for the Cluster. Cluster name requirements are 1-16 character length limit, Must start with a letter, Must be English letters, numbers, - only, No repeating hyphens, Must be unique within the region. Avoid entering confidential information.
	DisplayName *string `json:"displayName,omitempty" tf:"display_name,omitempty"`

	// The number of ESXi hosts to create in the Cluster. You can add more hosts later (see CreateEsxiHost).
	EsxiHostsCount *float64 `json:"esxiHostsCount,omitempty" tf:"esxi_hosts_count,omitempty"`

	// (Updatable) The ESXi software bundle to install on the ESXi hosts in the Cluster.  Only versions under the same vmwareSoftwareVersion and have been validate by Oracle Cloud VMware Solution will be accepted. To get a list of the available versions, use ListSupportedVmwareSoftwareVersions.
	EsxiSoftwareVersion *string `json:"esxiSoftwareVersion,omitempty" tf:"esxi_software_version,omitempty"`

	// (Updatable) Free-form tags for this resource. Each tag is a simple key-value pair with no predefined name, type, or namespace. For more information, see Resource Tags.  Example: {"Department": "Finance"}
	// +mapType=granular
	FreeformTags map[string]*string `json:"freeformTags,omitempty" tf:"freeform_tags,omitempty"`

	// The OCID of the Cluster.
	ID *string `json:"id,omitempty" tf:"id,omitempty"`

	// The billing option selected during Cluster creation. ListSupportedCommitments.
	InitialCommitment *string `json:"initialCommitment,omitempty" tf:"initial_commitment,omitempty"`

	// The initial OCPU count of the Cluster's ESXi hosts.
	InitialHostOcpuCount *float64 `json:"initialHostOcpuCount,omitempty" tf:"initial_host_ocpu_count,omitempty"`

	// The initial compute shape of the Cluster's ESXi hosts. ListSupportedHostShapes.
	InitialHostShapeName *string `json:"initialHostShapeName,omitempty" tf:"initial_host_shape_name,omitempty"`

	// A prefix used in the name of each ESXi host and Compute instance in the Cluster. If this isn't set, the Cluster's displayName is used as the prefix.
	InstanceDisplayNamePrefix *string `json:"instanceDisplayNamePrefix,omitempty" tf:"instance_display_name_prefix,omitempty"`

	// Indicates whether shielded instance is enabled for this Cluster.
	IsShieldedInstanceEnabled *bool `json:"isShieldedInstanceEnabled,omitempty" tf:"is_shielded_instance_enabled,omitempty"`

	// (Updatable) The network configurations used by Cluster, including OCIDs of the management subnet and VLANs.
	NetworkConfiguration []NetworkConfigurationObservation `json:"networkConfiguration,omitempty" tf:"network_configuration,omitempty"`

	// The OCID of the SDDC that the Cluster belongs to.
	SddcID *string `json:"sddcId,omitempty" tf:"sddc_id,omitempty"`

	// The current state of the Cluster.
	State *string `json:"state,omitempty" tf:"state,omitempty"`

	// The date and time the Cluster was created, in the format defined by RFC3339.  Example: 2016-08-25T21:10:29.600Z
	TimeCreated *string `json:"timeCreated,omitempty" tf:"time_created,omitempty"`

	// The date and time the Cluster was updated, in the format defined by RFC3339.
	TimeUpdated *string `json:"timeUpdated,omitempty" tf:"time_updated,omitempty"`

	// The vSphere licenses to use when upgrading the Cluster.
	UpgradeLicenses []UpgradeLicensesObservation `json:"upgradeLicenses,omitempty" tf:"upgrade_licenses,omitempty"`

	// (Updatable) The VMware software bundle to install on the ESXi hosts in the Cluster. To get a list of the available versions, use ListSupportedVmwareSoftwareVersions.
	VmwareSoftwareVersion *string `json:"vmwareSoftwareVersion,omitempty" tf:"vmware_software_version,omitempty"`

	// vSphere Cluster types.
	VsphereType *string `json:"vsphereType,omitempty" tf:"vsphere_type,omitempty"`

	// The links to binary objects needed to upgrade vSphere.
	VsphereUpgradeObjects []VsphereUpgradeObjectsObservation `json:"vsphereUpgradeObjects,omitempty" tf:"vsphere_upgrade_objects,omitempty"`

	// The CIDR block for the IP addresses that VMware VMs in the Cluster use to run application workloads.
	WorkloadNetworkCidr *string `json:"workloadNetworkCidr,omitempty" tf:"workload_network_cidr,omitempty"`
}

type OcvsClusterParameters struct {

	// The OCID of the Capacity Reservation.
	// +kubebuilder:validation:Optional
	CapacityReservationID *string `json:"capacityReservationId,omitempty" tf:"capacity_reservation_id,omitempty"`

	// The availability domain to create the Cluster's ESXi hosts in. For multi-AD Cluster deployment, set to multi-AD.
	// +kubebuilder:validation:Optional
	ComputeAvailabilityDomain *string `json:"computeAvailabilityDomain,omitempty" tf:"compute_availability_domain,omitempty"`

	// A list of datastore info for the Cluster. This value is required only when initialHostShapeName is a standard shape.
	// +kubebuilder:validation:Optional
	Datastores []DatastoresParameters `json:"datastores,omitempty" tf:"datastores,omitempty"`

	// (Updatable) Defined tags for this resource. Each key is predefined and scoped to a namespace. For more information, see Resource Tags.  Example: {"Operations.CostCenter": "42"}
	// +kubebuilder:validation:Optional
	// +mapType=granular
	DefinedTags map[string]*string `json:"definedTags,omitempty" tf:"defined_tags,omitempty"`

	// (Updatable) A descriptive name for the Cluster. Cluster name requirements are 1-16 character length limit, Must start with a letter, Must be English letters, numbers, - only, No repeating hyphens, Must be unique within the region. Avoid entering confidential information.
	// +kubebuilder:validation:Optional
	DisplayName *string `json:"displayName,omitempty" tf:"display_name,omitempty"`

	// The number of ESXi hosts to create in the Cluster. You can add more hosts later (see CreateEsxiHost).
	// +kubebuilder:validation:Optional
	EsxiHostsCount *float64 `json:"esxiHostsCount,omitempty" tf:"esxi_hosts_count,omitempty"`

	// (Updatable) The ESXi software bundle to install on the ESXi hosts in the Cluster.  Only versions under the same vmwareSoftwareVersion and have been validate by Oracle Cloud VMware Solution will be accepted. To get a list of the available versions, use ListSupportedVmwareSoftwareVersions.
	// +kubebuilder:validation:Optional
	EsxiSoftwareVersion *string `json:"esxiSoftwareVersion,omitempty" tf:"esxi_software_version,omitempty"`

	// (Updatable) Free-form tags for this resource. Each tag is a simple key-value pair with no predefined name, type, or namespace. For more information, see Resource Tags.  Example: {"Department": "Finance"}
	// +kubebuilder:validation:Optional
	// +mapType=granular
	FreeformTags map[string]*string `json:"freeformTags,omitempty" tf:"freeform_tags,omitempty"`

	// The billing option selected during Cluster creation. ListSupportedCommitments.
	// +kubebuilder:validation:Optional
	InitialCommitment *string `json:"initialCommitment,omitempty" tf:"initial_commitment,omitempty"`

	// The initial OCPU count of the Cluster's ESXi hosts.
	// +kubebuilder:validation:Optional
	InitialHostOcpuCount *float64 `json:"initialHostOcpuCount,omitempty" tf:"initial_host_ocpu_count,omitempty"`

	// The initial compute shape of the Cluster's ESXi hosts. ListSupportedHostShapes.
	// +kubebuilder:validation:Optional
	InitialHostShapeName *string `json:"initialHostShapeName,omitempty" tf:"initial_host_shape_name,omitempty"`

	// A prefix used in the name of each ESXi host and Compute instance in the Cluster. If this isn't set, the Cluster's displayName is used as the prefix.
	// +kubebuilder:validation:Optional
	InstanceDisplayNamePrefix *string `json:"instanceDisplayNamePrefix,omitempty" tf:"instance_display_name_prefix,omitempty"`

	// Indicates whether shielded instance is enabled for this Cluster.
	// +kubebuilder:validation:Optional
	IsShieldedInstanceEnabled *bool `json:"isShieldedInstanceEnabled,omitempty" tf:"is_shielded_instance_enabled,omitempty"`

	// (Updatable) The network configurations used by Cluster, including OCIDs of the management subnet and VLANs.
	// +kubebuilder:validation:Optional
	NetworkConfiguration []NetworkConfigurationParameters `json:"networkConfiguration,omitempty" tf:"network_configuration,omitempty"`

	// The OCID of the SDDC that the Cluster belongs to.
	// +kubebuilder:validation:Optional
	SddcID *string `json:"sddcId,omitempty" tf:"sddc_id,omitempty"`

	// (Updatable) The VMware software bundle to install on the ESXi hosts in the Cluster. To get a list of the available versions, use ListSupportedVmwareSoftwareVersions.
	// +kubebuilder:validation:Optional
	VmwareSoftwareVersion *string `json:"vmwareSoftwareVersion,omitempty" tf:"vmware_software_version,omitempty"`

	// The CIDR block for the IP addresses that VMware VMs in the Cluster use to run application workloads.
	// +kubebuilder:validation:Optional
	WorkloadNetworkCidr *string `json:"workloadNetworkCidr,omitempty" tf:"workload_network_cidr,omitempty"`
}

type UpgradeLicensesInitParameters struct {
}

type UpgradeLicensesObservation struct {

	// vSphere license key value.
	LicenseKey *string `json:"licenseKey,omitempty" tf:"license_key,omitempty"`

	// vSphere license type.
	LicenseType *string `json:"licenseType,omitempty" tf:"license_type,omitempty"`
}

type UpgradeLicensesParameters struct {
}

type VsphereUpgradeObjectsInitParameters struct {
}

type VsphereUpgradeObjectsObservation struct {

	// Binary object download link.
	DownloadLink *string `json:"downloadLink,omitempty" tf:"download_link,omitempty"`

	// Binary object description.
	LinkDescription *string `json:"linkDescription,omitempty" tf:"link_description,omitempty"`
}

type VsphereUpgradeObjectsParameters struct {
}

// OcvsClusterSpec defines the desired state of OcvsCluster
type OcvsClusterSpec struct {
	v1.ResourceSpec `json:",inline"`
	ForProvider     OcvsClusterParameters `json:"forProvider"`
	// THIS IS A BETA FIELD. It will be honored
	// unless the Management Policies feature flag is disabled.
	// InitProvider holds the same fields as ForProvider, with the exception
	// of Identifier and other resource reference fields. The fields that are
	// in InitProvider are merged into ForProvider when the resource is created.
	// The same fields are also added to the terraform ignore_changes hook, to
	// avoid updating them after creation. This is useful for fields that are
	// required on creation, but we do not desire to update them after creation,
	// for example because of an external controller is managing them, like an
	// autoscaler.
	InitProvider OcvsClusterInitParameters `json:"initProvider,omitempty"`
}

// OcvsClusterStatus defines the observed state of OcvsCluster.
type OcvsClusterStatus struct {
	v1.ResourceStatus `json:",inline"`
	AtProvider        OcvsClusterObservation `json:"atProvider,omitempty"`
}

// +kubebuilder:object:root=true
// +kubebuilder:subresource:status
// +kubebuilder:storageversion

// OcvsCluster is the Schema for the OcvsClusters API. Provides the Cluster resource in Oracle Cloud Infrastructure Oracle Cloud VMware Solution service
// +kubebuilder:printcolumn:name="SYNCED",type="string",JSONPath=".status.conditions[?(@.type=='Synced')].status"
// +kubebuilder:printcolumn:name="READY",type="string",JSONPath=".status.conditions[?(@.type=='Ready')].status"
// +kubebuilder:printcolumn:name="EXTERNAL-NAME",type="string",JSONPath=".metadata.annotations.crossplane\\.io/external-name"
// +kubebuilder:printcolumn:name="AGE",type="date",JSONPath=".metadata.creationTimestamp"
// +kubebuilder:resource:scope=Cluster,categories={crossplane,managed,oci}
type OcvsCluster struct {
	metav1.TypeMeta   `json:",inline"`
	metav1.ObjectMeta `json:"metadata,omitempty"`
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.computeAvailabilityDomain) || (has(self.initProvider) && has(self.initProvider.computeAvailabilityDomain))",message="spec.forProvider.computeAvailabilityDomain is a required parameter"
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.esxiHostsCount) || (has(self.initProvider) && has(self.initProvider.esxiHostsCount))",message="spec.forProvider.esxiHostsCount is a required parameter"
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.networkConfiguration) || (has(self.initProvider) && has(self.initProvider.networkConfiguration))",message="spec.forProvider.networkConfiguration is a required parameter"
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.sddcId) || (has(self.initProvider) && has(self.initProvider.sddcId))",message="spec.forProvider.sddcId is a required parameter"
	Spec   OcvsClusterSpec   `json:"spec"`
	Status OcvsClusterStatus `json:"status,omitempty"`
}

// +kubebuilder:object:root=true

// OcvsClusterList contains a list of OcvsClusters
type OcvsClusterList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []OcvsCluster `json:"items"`
}

// Repository type metadata.
var (
	OcvsCluster_Kind             = "OcvsCluster"
	OcvsCluster_GroupKind        = schema.GroupKind{Group: CRDGroup, Kind: OcvsCluster_Kind}.String()
	OcvsCluster_KindAPIVersion   = OcvsCluster_Kind + "." + CRDGroupVersion.String()
	OcvsCluster_GroupVersionKind = CRDGroupVersion.WithKind(OcvsCluster_Kind)
)

func init() {
	SchemeBuilder.Register(&OcvsCluster{}, &OcvsClusterList{})
}
