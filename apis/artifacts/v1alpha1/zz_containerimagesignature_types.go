...
type ContainerImageSignature struct {
	...
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.kmsKeyVersionId) || (has(self.initProvider) && has(self.initProvider.kmsKeyVersionId))",message="spec.forProvider.kmsKeyVersionId is a required parameter"
