...
type RemediationRecipe struct {
	...
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.knowledgeBaseId) || (has(self.initProvider) && has(self.initProvider.knowledgeBaseId))",message="spec.forProvider.knowledgeBaseId is a required parameter"
