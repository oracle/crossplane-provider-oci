...
type Verify struct {
	...
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.keyVersionId) || (has(self.initProvider) && has(self.initProvider.keyVersionId))",message="spec.forProvider.keyVersionId is a required parameter"
