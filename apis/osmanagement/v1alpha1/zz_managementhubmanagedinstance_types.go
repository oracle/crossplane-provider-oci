...
type ManagementHubManagedInstance struct {
	...
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.managedInstanceId) || (has(self.initProvider) && has(self.initProvider.managedInstanceId))",message="spec.forProvider.managedInstanceId is a required parameter"
