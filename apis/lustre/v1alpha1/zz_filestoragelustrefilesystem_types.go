...
type FileStorageLustreFileSystemInitParameters struct {
...
type FileStorageLustreFileSystemObservation struct {
...
type FileStorageLustreFileSystemParameters struct {
...
// FileStorageLustreFileSystemSpec defines the desired state of FileStorageLustreFileSystem
type FileStorageLustreFileSystemSpec struct {
	v1.ResourceSpec `json:",inline"`
	ForProvider     FileStorageLustreFileSystemParameters `json:"forProvider"`
	...
	// autoscaler.
	InitProvider FileStorageLustreFileSystemInitParameters `json:"initProvider,omitempty"`
}

// FileStorageLustreFileSystemStatus defines the observed state of FileStorageLustreFileSystem.
type FileStorageLustreFileSystemStatus struct {
	v1.ResourceStatus `json:",inline"`
	AtProvider        FileStorageLustreFileSystemObservation `json:"atProvider,omitempty"`
}
...
// FileStorageLustreFileSystem is the Schema for the FileStorageLustreFileSystems API. Provides the Lustre File System resource in Oracle Cloud Infrastructure Lustre File Storage service
...
type FileStorageLustreFileSystem struct {
	...
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.fileSystemName) || (has(self.initProvider) && has(self.initProvider.fileSystemName))",message="spec.forProvider.fileSystemName is a required parameter"
	...
	Spec   FileStorageLustreFileSystemSpec   `json:"spec"`
	Status FileStorageLustreFileSystemStatus `json:"status,omitempty"`
}

// +kubebuilder:object:root=true

// FileStorageLustreFileSystemList contains a list of FileStorageLustreFileSystems
type FileStorageLustreFileSystemList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []FileStorageLustreFileSystem `json:"items"`
}

// Repository type metadata.
var (
	FileStorageLustreFileSystem_Kind             = "FileStorageLustreFileSystem"
	FileStorageLustreFileSystem_GroupKind        = schema.GroupKind{Group: CRDGroup, Kind: FileStorageLustreFileSystem_Kind}.String()
	FileStorageLustreFileSystem_KindAPIVersion   = FileStorageLustreFileSystem_Kind + "." + CRDGroupVersion.String()
	FileStorageLustreFileSystem_GroupVersionKind = CRDGroupVersion.WithKind(FileStorageLustreFileSystem_Kind)
)

func init() {
	SchemeBuilder.Register(&FileStorageLustreFileSystem{}, &FileStorageLustreFileSystemList{})
}
