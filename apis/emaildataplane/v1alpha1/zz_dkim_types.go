...
type Dkim struct {
	...
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.emailDomainId) || (has(self.initProvider) && has(self.initProvider.emailDomainId))",message="spec.forProvider.emailDomainId is a required parameter"
