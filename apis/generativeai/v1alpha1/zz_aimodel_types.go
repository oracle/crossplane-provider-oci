...
type AiModelInitParameters struct {
...
type AiModelObservation struct {
...
type FineTuneDetailsParameters struct {
	...
	// The OCID of the dedicated AI cluster this fine-tuning runs on.
	// +kubebuilder:validation:Optional
	DedicatedAIClusterID *string `json:"dedicatedAiClusterId" tf:"dedicated_ai_cluster_id,omitempty"`
...
type AiModelSpec struct {
	...
	ForProvider     AiModelParameters `json:"forProvider"`
	...
	// autoscaler.
	InitProvider AiModelInitParameters `json:"initProvider,omitempty"`
}

// AiModelStatus defines the observed state of AiModel.
type AiModelStatus struct {
	v1.ResourceStatus `json:",inline"`
	AtProvider        AiModelObservation `json:"atProvider,omitempty"`
}
...
// AiModel is the Schema for the AiModels API. Provides the Model resource in Oracle Cloud Infrastructure Generative AI service
...
type AiModel struct {
	...
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.baseModelId) || (has(self.initProvider) && has(self.initProvider.baseModelId))",message="spec.forProvider.baseModelId is a required parameter"
...
// AiModelList contains a list of AiModels
...
var (
	AiModel_Kind             = "AiModel"
	AiModel_GroupKind        = schema.GroupKind{Group: CRDGroup, Kind: AiModel_Kind}.String()
	AiModel_KindAPIVersion   = AiModel_Kind + "." + CRDGroupVersion.String()
	AiModel_GroupVersionKind = CRDGroupVersion.WithKind(AiModel_Kind)
)
