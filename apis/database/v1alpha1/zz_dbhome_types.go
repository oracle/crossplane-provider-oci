...
type DbHomeDatabaseDBBackupConfigBackupDestinationDetailsInitParameters struct {
	...
	// (Applicable when source=NONE | VM_CLUSTER_NEW) Indicates whether the backup destination is cross-region or local region.
	IsRemote *bool `json:"isRemote,omitempty" tf:"is_remote,omitempty"`

	// (Applicable when source=NONE | VM_CLUSTER_NEW) The name of the remote region where the remote automatic incremental backups will be stored.
...
type DbHomeDatabaseDBBackupConfigInitParameters struct {
	...
	// (Applicable when source=NONE | VM_CLUSTER_NEW) This defines when the backups will be deleted. - IMMEDIATE option keep the backup for predefined time i.e 72 hours and then delete permanently... - RETAIN will keep the backups as per the policy defined for database backups.
...
type DbHomeDatabaseInitParameters struct {
	...
	// (Applicable when source=NONE | VM_CLUSTER_NEW) The OCID of the key container version that is used in database transparent data encryption (TDE) operations KMS Key can have multiple key versions. If none is specified, the current key version (latest) of the Key Id is used for the operation. Autonomous Database Serverless does not use key versions, hence is not applicable for Autonomous Database Serverless instances.
