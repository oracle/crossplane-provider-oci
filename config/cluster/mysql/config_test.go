...
func TestConfigureBlueGreenDeployment(t *testing.T) {
	r := &config.Resource{}
	configureBlueGreenDeployment(r)
	...
	if !reflect.DeepEqual(r.OverrideFieldNames, want) {
		t.Fatalf("OverrideFieldNames = %#v, want %#v", r.OverrideFieldNames, want)
	}
}
